"""Krea 2 NegPiP encoding and diffusion patches.

Vendored and adapted from blue-pen5805/ComfyUI-krea2-negpip at upstream
commit 3740add9dbdc9f254a2befda30e95ba95e3b115d under the GNU Affero
General Public License v3.0.

The upstream standalone node registration is intentionally omitted. This
module exposes :func:`patch_krea2_negpip` for the package's combined node.
"""
#
...
def _patch_clip_for_krea2_negpip(clip):
    ...
    if not hasattr(patched_cond_stage_model, "_krea2_negpip_original_encode_token_weights"):
        patched_cond_stage_model._krea2_negpip_original_encode_token_weights = (
            patched_cond_stage_model.encode_token_weights
        )
...
def krea2_negpip_wrapper(
    ...
    **kwargs,
    ...
    if negative_positions is None:
        ...
        if trim_to_length is not None and trim_to_length < int(context.shape[1]):
            ...
            if (
                ...
                and int(attention_mask.shape[-1]) == int(original_context_len)
            ):
    ...
    # The cloned ModelPatcher still shares its underlying diffusion modules.
    # Serialize the temporary forward replacements so overlapping executions
    # cannot restore one another's patches halfway through a model call.
...
def validate_krea2_negpip_conditioning(conditioning) -> None:
    """Require an encoded Krea 2 conditioning tensor with negative positions."""
    ...
    for item in conditioning:
        ...
        if isinstance(metadata, dict):
            positions = metadata.get(NEGATIVE_POSITIONS_EXTRA_KEY)
        ...
        _, negative_positions, _ = _parse_and_strip_sidecar_full(context)
        if negative_positions is not None and any(negative_positions):
            return

    raise RuntimeError(
        "The Krea 2 negative prompt was compiled, but its NegPiP sidecar "
        "and metadata were lost during CLIP encoding. Use CLIPLoader type "
        "'krea2' and do not route compiled_prompt through another "
        "text-encode node."
    )


def patch_krea2_negpip(
    model,
    clip,
    *,
    value_strength=1.0,
    patch_txtfusion_refiners=True,
    block_start=0,
    block_end=27,
    block_stride=1,
):
    """Clone and patch a matching Krea 2 model and Qwen3-VL CLIP."""

    try:
        diffusion_model = model.get_model_object("diffusion_model")
    except Exception as exc:
        raise RuntimeError(
            "Krea 2 NegPiP could not access the connected diffusion model."
        ) from exc
    if not _is_krea2_dm(diffusion_model):
        raise RuntimeError(
            "Krea 2 NegPiP requires the native ComfyUI Krea 2 diffusion layout."
        )
    ...
    if previous_calc is None:
        ...
        patched.model_options["sampler_calc_cond_batch_function"] = (
            _make_krea2_negpip_calc_cond_batch(previous_calc)
        )
    ...
    diffusion_wrappers = to["wrappers"][
        comfy.patcher_extension.WrappersMP.DIFFUSION_MODEL
    ]
...
__all__ = [
    "KREA2_TEXT_ENCODER_KEY",
    "NEGATIVE_POSITIONS_EXTRA_KEY",
    "patch_krea2_negpip",
